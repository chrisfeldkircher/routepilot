...
function defaultState(): ImportStateShape {
  return {
    ...
    errors: [
      {
        id: 1, row: null, column: 'email', type: 'missing-mapping',
        message: 'Required column "email" is not mapped to any target field', resolved: false,
      },
      {
        id: 2, row: 14, column: 'age', type: 'type-mismatch',
        message: 'Expected number, got "twenty-three"', value: 'twenty-three', resolved: false,
      },
      {
        id: 3, row: 42, column: 'employee_id', type: 'duplicate-key',
        message: 'Duplicate key "E-1047" — also in row 7', value: 'E-1047', resolved: false,
      },
    ],
    mappings: [
...
class ImportStateManager {
  ...
  setMapping(source: string, target: string | null): void {
    const m = this.init().mappings.find((mp) => mp.source === source);
