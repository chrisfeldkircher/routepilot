...
@Component({
  selector: 'rp-tour-assistant-button',
  standalone: true,
  ...
  template: `
    <button
      ...
      class="tour-btn tour-btn-secondary tour-assistant-nav-btn gt-inline-flex gt-items-center gt-justify-center gt-rounded-md gt-border gt-border-border gt-px-2.5 gt-py-1.5 gt-text-xs gt-font-medium gt-text-muted-foreground hover:gt-bg-accent"
      [attr.aria-expanded]="assistant.open()"
      [attr.aria-label]="ariaLabel"
      [attr.title]="title"
      [attr.data-open]="assistant.open() ? 'true' : 'false'"
      data-tour="tour-assistant-button"
    >
      <svg
        class="tour-assistant-nav-btn-icon"
        viewBox="0 0 24 24"
        width="16"
        height="16"
        fill="currentColor"
...
export class TourAssistantButtonComponent {
  @Input() ariaLabel = 'Ask assistant';
  @Input() title = 'Ask assistant';
