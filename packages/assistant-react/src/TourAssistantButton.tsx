...
export interface TourAssistantButtonProps {
  ariaLabel?: string;
  title?: string;
}

export function TourAssistantButton({
  ariaLabel = 'Ask assistant',
  title = 'Ask assistant',
}: TourAssistantButtonProps = {}): JSX.Element {
  const { open, toggle } = useTourAssistantContext();
  return (
    <button
      type="button"
      onClick={toggle}
      className="tour-btn tour-btn-secondary tour-assistant-nav-btn gt-inline-flex gt-items-center gt-justify-center gt-rounded-md gt-border gt-border-border gt-px-2.5 gt-py-1.5 gt-text-xs gt-font-medium gt-text-muted-foreground hover:gt-bg-accent"
      aria-expanded={open}
      aria-label={ariaLabel}
      title={title}
      data-open={open ? 'true' : 'false'}
      data-tour="tour-assistant-button"
    >
      <AssistantIcon />
    </button>
  );
...
function AssistantIcon(): JSX.Element {
  return (
    <svg
      className="tour-assistant-nav-btn-icon"
      viewBox="0 0 24 24"
      width="16"
      height="16"
      fill="currentColor"
...
export default TourAssistantButton;
