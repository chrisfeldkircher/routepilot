...
import type { DagTourNode } from '../core/dagTourHistory';
...
const GuidedTourOverlay = (): JSX.Element | null => {
  ...
  }, [state.status, currentStep, routeGuard]);
  ...
  const rawBody = (content?.body as ReactNode | undefined) ?? currentStep.body ?? '';
  const body = parseInlineMarkup(rawBody);
  const hint = (content as StepContent | null | undefined)?.hint;
  const media = (content as StepContent | null | undefined)?.media;
