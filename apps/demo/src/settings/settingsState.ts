...
class SettingsStateManager {
  ...
  setEmailDigest(value: EmailDigest): void {
    this.init().emailDigest = value;
  }

  setSlackChannel(channel: string | null): void {
    this.init().slackChannel = channel;
  }

  setPushEnabled(enabled: boolean): void {
    this.init().pushEnabled = enabled;
  }

  setQuietHours(enabled: boolean): void {
    this.init().quietHoursEnabled = enabled;
  }
  ...
  setTwoFactor(enabled: boolean): void {
    this.init().twoFactorEnabled = enabled;
  }

  setSessionTimeout(minutes: number): void {
    this.init().sessionTimeoutMin = minutes;
  }

  setIpAllowlist(ips: string[]): void {
    this.init().ipAllowlist = ips;
  }

  setIssueTransition(mode: IssueTransition): void {
    this.init().issueTransition = mode;
  }

  setRequireApproval(required: boolean): void {
    this.init().requireApproval = required;
  }
  ...
  setApiKeyRevealed(revealed: boolean): void {
    this.init().apiKeyRevealed = revealed;
  }

  setWebhookUrl(url: string): void {
    this.init().webhookUrl = url;
  }

  setWebhookEvents(events: string[]): void {
    this.init().webhookEvents = events;
  }
}
